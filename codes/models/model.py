...
class GPNERModel(nn.Module):
    def __init__(self, encoder_class, args):
        ...
        entity_class_num = args.num_labels
        print('Entity class num:{}'.format(entity_class_num))
...
class GPFilterModel(nn.Module):
    def __init__(self, encoder_class, args):
        ...
        self.s_o_head = GlobalPointer(hiddensize=hiddensize, ent_type_size=args.num_predicates, inner_dim=args.inner_dim,
                                      RoPE=False, tril_mask=False, do_rdrop=args.do_rdrop, dropout=args.dropout).to(args.device)
        self.s_o_tail = GlobalPointer(hiddensize=hiddensize, ent_type_size=args.num_predicates, inner_dim=args.inner_dim,
                                      RoPE=False, tril_mask=False, do_rdrop=args.do_rdrop, dropout=args.dropout).to(args.device)

        print('num_predicates')
        print(args.num_predicates)
    def forward(self, batch_token_ids, batch_mask_ids):
        outputs = self.encoder(batch_token_ids, batch_mask_ids)[0]
